...
const REORG_SAFETY_BLOCK_AGE = 100; // Try to get an injected web3 provider, return a public one otherwise.
...
class Aragon {
  ...
  async init(options = {}) {
    ...
    this.trigger = new _rxjs.Subject();
  ...
  async initAcl({
    ...
    const ACL_CACHE_KEY = (0, _utils.getCacheKey)(aclAddress, 'acl'); // Check if we have cached ACL for this address
  ...
  initApps() {
    ...
    (0, _operators.concatMap)(async ({
      ...
      .reverse().find(version => version.contractAddress === installedRepoInfo.contractAddress); // Get info for the current and latest versions of the repo

      const currentVersionRequest = applicationInfoCache.request("".concat(appId, ".").concat(currentVersion.contractAddress)).catch(() => ({})).then(content => ({
        content,
        version: currentVersion.version
      }));
      const versionInfos = await Promise.all([currentVersionRequest, currentVersion.contractAddress === latestVersion.contractAddress ? currentVersionRequest // current version is also the latest, no need to refetch
      : applicationInfoCache.request("".concat(appId, ".").concat(latestVersion.contractAddress)).catch(() => ({})).then(content => ({
        content,
  ...
  async initAppMetadata({
    cacheBlockHeight = 0
  } = {}) {
    const cachedMetadata = (await this.cache.get('appMetadata')) || {};
    this.appMetadata = new _rxjs.BehaviorSubject(_objectSpread({}, cachedMetadata)).pipe((0, _operators.scan)((inMemoryMetadata, {
      from,
      blockNumber,
      dataId,
      cid,
      ...
      function mutate(metadataRegistry) {
        const key = "".concat(from, ",").concat(dataId);
  ...
  async initForwardedActions({
    ...
    this.forwardedActions = new _rxjs.BehaviorSubject(_objectSpread({}, cachedActions)).pipe((0, _operators.scan)((inMemoryActions, {
      actionId,
      blockNumber,
      currentApp,
      evmScript,
      ...
      if (!currentApp && !actionId && !evmScript && !target || !['pending', 'failed', 'completed'].includes(status) || !blockNumber || !target) return inMemoryActions;
      ...
      function mutate(actions) {
        ...
        const actionKey = "".concat(currentApp, ",").concat(actionId);
        const existingAction = targetActions.actions[actionKey]; // dealing with async:
        ...
        actions[target].actions[actionKey] = {
          actionId,
          currentApp,
          evmScript,
          status,
        ...
        if (!existingAction) {
          const statusKey = "".concat(status, "ActionKeys");
  ...
  setForwardedAction({
    ...
    if (!['pending', 'failed', 'completed'].includes(status)) {
      throw new Error("unexpected status for forwardedAction\n         expected one of 'pending', 'failed', 'completed'\n         got: '".concat(status, "'"));
    }

    if (!evmScript) {
      throw new Error("must provide a valid evmScript when forwarding an action;\n         got: '".concat(evmScript, "'"));
  ...
  async runApp(proxyAddress) {
    ...
    return sandboxMessengerProvider => {
      ...
      handlers.createRequestHandler(request$, 'accounts', handlers.accounts), handlers.createRequestHandler(request$, 'cache', handlers.cache), handlers.createRequestHandler(request$, 'describe_script', handlers.describeScript), handlers.createRequestHandler(request$, 'describe_transaction', handlers.describeTransaction), handlers.createRequestHandler(request$, 'get_apps', handlers.getApps), handlers.createRequestHandler(request$, 'network', handlers.network), handlers.createRequestHandler(request$, 'path', handlers.path), handlers.createRequestHandler(request$, 'web3_eth', handlers.web3Eth), // Contract handlers
      ...
      handlers.createRequestHandler(request$, 'identify', handlers.appIdentifier), handlers.createRequestHandler(request$, 'address_identity', handlers.addressIdentity), handlers.createRequestHandler(request$, 'search_identities', handlers.searchIdentities), // Cross-app handlers
      handlers.createRequestHandler(request$, 'register_app_metadata', handlers.registerAppMetadata), handlers.createRequestHandler(request$, 'get_app_metadata', handlers.getAppMetadata), handlers.createRequestHandler(request$, 'query_app_metadata', handlers.queryAppMetadata), handlers.createRequestHandler(request$, 'update_forwarded_action', handlers.updateForwardedAction), handlers.createRequestHandler(request$, 'get_forwarded_actions', handlers.getForwardedActions), // Etc.
      handlers.createRequestHandler(request$, 'trigger', handlers.newTrigger), handlers.createRequestHandler(request$, 'getTriggers', handlers.getTriggers)).subscribe(response => messenger.sendResponse(response.id, response.payload)); // The attached unsubscribe isn't automatically bound to the subscription
  ...
  async getTransactionPath(destination, methodName, params, finalForwarder) {
    ...
    for (let account of accounts) {
      const path = await this.calculateTransactionPath(account, destination, methodName, params, finalForwarder);
  ...
  async getTransactionPathForIntentBasket(intentBasket, {
    ...
    : [];
    const intentPaths = await Promise.all(intentsToCheck.map(([destination, methodName, params]) => this.getTransactionPath(destination, methodName, params))); // If the paths don't match, we can't send the transactions in this intent basket together
    ...
    if (pathsMatch) {
      ...
      const directTransactions = await Promise.all(intentBasket.map(async ([destination, methodName, params]) => (0, _transactions.createDirectTransactionForApp)(sender, (await this.getApp(destination)), methodName, params, this.web3)));
  ...
  async calculateTransactionPath(sender, destination, methodName, params, finalForwarder) {
    ...
    const directTransaction = await (0, _transactions.createDirectTransactionForApp)(sender, app, methodName, params, this.web3);
    let appsWithPermissionForMethod = []; // Only try to perform direct transaction if no final forwarder is provided or
    ...
    if (!finalForwarderProvided || finalForwarder === sender) {
      ...
      const method = methods.find(method => method.sig.split('(')[0] === methodName);

      if (!method) {
        throw new Error("No method named ".concat(methodName, " on ").concat(destination));
      ...
      appsWithPermissionForMethod = _dotProp.default.get(permissionsForDestination, "".concat(roleSig, ".allowedEntities"), []); // No one has access, so of course we (or the final forwarder) don't as well
    ...
    if (finalForwarderProvided) {
      ...
      forwardersWithPermission = forwarders.filter(forwarder => (0, _utils.includesAddress)(appsWithPermissionForMethod, forwarder));
    }
  ...
  async calculateForwardingPath(sender, destination, directTransaction, forwardersWithPermission) {
    ...
    for (const forwarder of forwardersWithPermission) {
      ...
      if (await this.canForward(forwarder, sender, script)) {
        ...
        try {
          const transactionWithFee = await (0, _transactions.applyForwardingPretransaction)(transaction, this.web3); // `applyTransactionGas` can throw if the transaction will fail
